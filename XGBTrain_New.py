...
from XGBLib import create_tier2daily_features
from XGBLib import create_tier2hourly_features
...
feat_dict = {'DA_RT': 6, 'FLOAD': 8, 'FTEMP': 20, 'OUTAGE': 12}   # Number Of Top Features To Use If Reading From Dict
...
num_targets = 1000                                                # Number of Targets To Train (Large Number = Train All Targets In File)
nrounds = 5000                                                    # Max Rounds To Train
early_stopping=10                                                 # Early Train Stopping
exp_folds = 10                                                     # Number of Exps To Do For Each Senario (To Take Median And SD Of)
...
gpu_train = True                                                  # Train Using GPU (Default is CPU)
...
if iso == 'MISO':
    tier2_backtest_filename = 'Backtest_12092019_Master_Nodes_Dataset_Dict_MISO_EXP10_'
    tier2_PnL_filename = 'PnL_Results_Backtest_12092019_Master_Nodes_Dataset_Dict_MISO_EXP10__SD1000_notscaled_nocaps_alllocs_1.25_1.00'
    tier2_hypergrid_name = 'Gridsearch_daily_Tier2_daily_Backtest_12092019_Master_Nodes_Dataset_Dict_MISO_EXP10__MISO_AECI.APM_2.AZ_DART_SD1000_PnL'
elif iso == 'PJM':
    tier2_backtest_filename = 'Backtest_12092019_Master_Nodes_Dataset_Dict_PJM_EXP10_'
    tier2_PnL_filename = 'PnL_Results_Backtest_12092019_Master_Nodes_Dataset_Dict_PJM_EXP10__SD1000_notscaled_nocaps_alllocs_0.75_0.75'
    tier2_hypergrid_name = 'Gridsearch_daily_Tier2_daily_Backtest_12092019_Master_Nodes_Dataset_Dict_PJM_EXP10__PJM_50485_DART_SD1000_PnL'



tier2_cat_vars = ['Month', 'Weekday']
tier2_dart_sd_location_filter = 'SD1000'
tier2_sd_limit_range = [1000,6]
tier2_model_creation_sd = 6
tier2_exp_folds = 15
tier2_cv_folds = 4
tier2_num_grids = 1
...
if input_file_type.upper() == 'DICT':
    if iso=='PJM':
        ...
        hypergrid_name = 'Gridsearch_2020_01_05_BACKTEST_DATA_DICT_MASTER_MISO_SD1000__MISO_AECI.ALTW_DART' #Filename of Stored Hypergrid From Gridsearch
        hypergrid_dict_name = 'GridsearchDict_2020_01_05_BACKTEST_DATA_DICT_MASTER_MISO_SD1000_'
    elif iso == 'SPP':
        ...
        hypergrid_name = 'Gridsearch_2020_01_05_BACKTEST_DATA_DICT_MASTER_ISONE_SD1000__ISONE_10033_DART'  # Filename of Stored Hypergrid From Gridsearch
    elif iso == 'NYISO':
...
def do_create_models(input_filename, save_name, iso, feat_dict, input_file_type, cat_vars,  exp_folds, hypergrid_name,hypergrid_dict_name, num_grids, gpu_train, nrounds, early_stopping, model_creation_sd, static_directory, working_directory):
...
def do_backtest(input_filename, save_name, num_targets, iso, feat_dict, input_file_type, cat_vars, start_date, sd_limit_range, exp_folds, cv_folds, hypergrid_name,hypergrid_dict_name, num_grids, gpu_train, nrounds, early_stopping, static_directory, working_directory, verbose=True):
    ...
    if cv_folds>1: preds_df = pd.DataFrame(index=master_df.index)
    else:
        test_df = master_df[master_df.index.get_level_values('Date') >= start_date]
    ...
    for target in targets:
        ...
        ##############################################################################################
        ### DEFINED TIME RANGE BACKTEST CODE Create Folds for CV Backtest If Number Of CV Folds >0 ###
        ##############################################################################################
        ...
        if cv_folds <= 1:
            ...
            train_df = feature_df[feature_df.index.get_level_values('Date') < start_date]
            test_df = feature_df[feature_df.index.get_level_values('Date') >= start_date]
            ...
            # If Multiple SD Ranges Train A Model For Each
            sd_fold = 1
            sd_preds_df = pd.DataFrame(index=test_df.index)

            for sd_limit in sd_limit_range:

                # Train Exps for Each Exp Fold (Folds Constant Across SD Ranges)
                exp_fold = 1
                exp_preds_df = pd.DataFrame(index=test_df.index)
                for exp_train_index, exp_test_index in exp_cv.split(train_df):
                    train_exp, eval_exp = train_df.iloc[exp_train_index], train_df.iloc[exp_test_index]
                    ...
                    # train_exp.to_csv('train_new' + str(exp_fold) + '.csv')
                    # test_df.to_csv('test_new' + str(exp_fold) + '.csv')
                    # eval_exp.to_csv('eval_new' + str(exp_fold) + '.csv')
                ...
                sd_preds_df[target + '_SD' + str(sd_limit) + '_pred'] = exp_preds_df.median(axis=1)
                sd_preds_df[target + '_SD' + str(sd_limit) + '_sd'] = exp_preds_df.std(axis=1)
                sd_preds_df[target + '_SD' + str(sd_limit) + '_act'] = test_df[target]
                sd_fold += 1

            # Add Each Successive Target To The Final Pred DF
            preds_df = pd.merge(preds_df, sd_preds_df, how='outer', on=['Date', 'HE'])
            preds_df.to_csv(backtest_directory+'Backtest_'+save_name+'.csv')
            target_num += 1

    return preds_df

def do_gridsearch(input_filename, save_name, iso, feat_dict, input_file_type, cat_vars,hypergrid_dict_name, sd_limit, cv_folds, gpu_train, nrounds, iterations, static_directory, working_directory, verbose=True):
...
def do_tier2_gridsearch(backtest_filename, PnL_filename, save_name, cat_vars, sd_limit, cv_folds, gpu_train, nrounds, iterations, hourly_daily,dart_sd_location_filter, static_directory, working_directory, verbose=True):
    # COORDINATES THE GRIDSEARCH(S)
    ...
    if hourly_daily == 'hourly':
        ...
        targets = [col for col in daily_PnL_df.columns if 'Total' not in col]

    else: print ('Check HourlyDaily Input Variable for Spelling')

    target_num = 1
    for target in targets[2:3]:
        print('Gridsearch Target: '+target+ '  ***GPU Compute= '+str(gpu_train))
        ...
        try:
            hypergrid.to_csv(gridsearch_directory+'Gridsearch_'+hourly_daily+'_' + save_name + '_' + target + '.csv', index=False)
            # hypergrids.update({target: hypergrid})
            # save_obj(hypergrids, gridsearch_directory+'GridsearchDict_'+hourly_daily+'_'  + save_name)
        except:
            hypergrid.to_csv(gridsearch_directory+'Gridsearch1_' + hourly_daily + '_' + save_name + '_' + target + '.csv', index=False)
            # hypergrids.update({target: hypergrid})
            # save_obj(hypergrids, gridsearch_directory+'GridsearchDict1_' + hourly_daily + '_' + save_name)

        target_num += 1

    return hypergrids

def do_tier2_backtest(backtest_filename,PnL_filename, save_name, cat_vars, sd_limit_range, exp_folds, cv_folds, hypergrid_name, num_grids, gpu_train, nrounds, early_stopping,hourly_daily,dart_sd_location_filter, working_directory, verbose=True):
    backtest_directory = working_directory + '\BacktestFiles\\'
    PnL_directory = working_directory + '\PnLFiles\\'
    gridsearch_directory = working_directory + '\GridsearchFiles\\'
    backtest_df = pd.read_csv(backtest_directory+backtest_filename + '.csv',index_col=['Date','HE'],parse_dates=True)
    ...
    if hourly_daily == 'hourly':
        ...
        targets = [col for col in daily_PnL_df.columns if 'Total' not in col]
        preds_df = pd.DataFrame(index=daily_PnL_df.index)
    else: print ('Check HourlyDaily Input Variable for Spelling')
    ...
    for target in targets:
        ...
        if cv_folds > 1:
            ...
            for cv_train_index, cv_test_index in kf_cv.split(feature_df):
                ...
                for sd_limit in sd_limit_range:
                    ...
                    for exp_train_index, exp_test_index in exp_cv.split(train_cv_df):
                        ...
                        if hourly_daily == 'daily': exp_preds_df = exp_preds_df.join(pred_df, on=['Date'])
                        elif hourly_daily == 'hourly': exp_preds_df = exp_preds_df.join(pred_df, on=['Date','HE'])
            ...
            # Add Each Successive Target To The Final Pred DF
            if hourly_daily == 'daily':
                preds_df = pd.merge(preds_df, cv_preds_df, how='outer', on=['Date'])
            ...
            preds_df.to_csv(backtest_directory+'Backtest_'+hourly_daily+'_' + save_name + '.csv')
            target_num += 1
    # preds_df = preds_df.join(daily_mw_df, how='inner')
    # preds_df.to_csv(save_directory + 'Backtest_' + hourly_daily + '_' + save_name + '.csv')

def do_tier2_create_models(backtest_filename,PnL_filename, save_name, cat_vars, exp_folds, hypergrid_name, num_grids, gpu_train, nrounds, early_stopping,hourly_daily,dart_sd_location_filter, tier2_model_creation_sd, static_directory):
    ...
    if hourly_daily == 'hourly':
        ...
        daily_PnL_df.columns = [col.replace('_SD1','').replace('_SD2','').replace('_SD3','').replace('_SD4','').replace('_SD4.5','').replace('_SD5','').replace('_SD5.5','').replace('_SD6','').replace('_SD6.5','').replace('_SD7','').replace('_SD7.5','').replace('_SD1000','') for col in daily_PnL_df.columns]
        targets = [col for col in daily_PnL_df.columns if 'Total' not in col]
    else: print ('Check HourlyDaily Input Variable for Spelling')

    # targets_df = pd.read_csv('leftovernodes.csv')
    # targets = targets_df['LeftoverNodes']
    # print(targets)
...
if run_gridsearch:
    gridsearch_save_name = input_file_name + '_' + iso + '_SD' + str(gridsearch_sd_limit) + '_' + name_adder
...
if run_backtest:
    backtest_save_name = input_file_name + '_' + iso + '_EXP' + str(exp_folds) + '_' + name_adder
...
if run_create_models:
    backtest_save_name = input_file_name + '_' + iso
...
if run_tier2_gridsearch:
    tier2_save_name = 'Tier2_'+tier2_hourly_daily+'_'+tier2_backtest_filename
